...
def checksum(path, name=_DEFAULT_HASH, size=_SIZE):
    ...
    Args:
        ...
        size: Optional positive integer number of bytes to read from the file
            at a time. Defaults to _SIZE.
    Returns:
    ...
    if not isinstance(size, int):
        raise TypeError('size must be a positive integer > 1024.')
    if size <= 1024:
        raise ValueError('size must be a positive integer > 1024.')
    ...
    def test_checksum(self):
        ...
        for value in [None, 42, []]:
            ...
            if not isinstance(value, int):
                self.assertRaises(TypeError, checksum, 'checksum.py',
                                  size=value)
        for value in ['', 'foobar']:
...
if __name__ == '__main__':
    ...
    if len(args.paths) <= 0:
        ...
        paths.sort()
        for path in paths:
            print('{0}  {1}'.format(checksum(path, hash_name), path))
